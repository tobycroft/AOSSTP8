...
function initClickCaptcha(options) {
    ...
    if (container) {
        container.addEventListener('click', function(e) {
            ...
            if (clickCountEl) {
                clickCountEl.textContent = remaining > 0 ? '还需点击 ' + remaining + ' 个' : '正在验证...';
            }

            if (userClicks.length >= targetCount) {
                verifyClicks();
            }
        });
    }
    ...
    if (reloadBtn) {
        reloadBtn.addEventListener('click', function() {
            ...
            generateCaptcha();
            setTimeout(() => { reloadBtn.disabled = false; }, 500);
        });
    }
    ...
    function generateCaptcha() {
        ...
        fetch(opts.apiUrl + '/click/create?t=' + Date.now(), {
            method: 'POST',
            body: formData,
            cache: 'no-store'
        })
    ...
    function verifyClicks() {
        ...
        .then(data => {
            if (data.code === 0) {
                ...
                if (statusEl) {
                    ...
                    statusEl.textContent = (data.echo || '验证失败') + '，正在自动刷新...';
                }
        ...
        .catch(error => {
            if (statusEl) {
                statusEl.className = 'click-status error';
                statusEl.textContent = '网络错误，正在自动刷新...';
            }
            opts.onError('网络错误: ' + error.message);
            setTimeout(() => {
                generateCaptcha();
